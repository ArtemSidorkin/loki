...
{
	protected class CCoreJavaUnit(generationContext:CGenerationContextJava, ruleContext:UnitContext)
		extends CCoreJava(generationContext, ruleContext) with CMixinRuleGenerationUnit
	{
		override def enter()
		{
			pushFrame()
	...
	override def getCore(generationContext:CGenerationContextJava, ruleContext:UnitContext) =
		new CCoreJavaUnit(generationContext, ruleContext)
}
