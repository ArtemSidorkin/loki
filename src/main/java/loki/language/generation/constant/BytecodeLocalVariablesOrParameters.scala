package loki.language.generation.constant

object BytecodeLocalVariablesOrParameters
{
	val UNIT__METHOD__CALL__PARAMETER__HOST = 1
	val UNIT__METHOD__CALL__PARAMETER__PARAMETERS = 2
	val UNIT__METHOD__CALL__VARIABLE__UNIT_CONTEXT = 3

	val UNIT_HEIR__METHOD__INIT__PARAMETER__TYPE = 1
	val UNIT_HEIR__METHOD__INIT__PARAMETER__UNIT_CONTEXT = 2
	val UNIT_HEIR__METHOD__INIT__PARAMETER__PARAMETERS = 3

	val MODULE_HEIR__METHOD__INIT__PARAMETER__UNIT_CONTEXT = 1
}
